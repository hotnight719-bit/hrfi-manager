    ...
    const handleSaveLog = async () => {
        ...
        const logData: Omit<WorkLog, 'id'> = {
            ...
            total_payment_to_workers: isManualWaiting
                ...
                : selectedWorkerIds.reduce((sum, wid) => sum + (workerPayments[wid] ?? finalPayPerWorker), 0),
                        ...
                        {selectedRate && (
                            <div className="mt-4 space-y-6">
                                <div className="grid grid-cols-2 gap-6">
                                    ...
                                    <div>
                                        <label className="block text-sm font-medium text-gray-700 mb-1">청구 옵션 (비율/직접입력)</label>
                                        <select
                                            className="w-full border rounded-md p-2"
                                            value={isManualWaiting ? -1 : (status === 'Normal' ? 1.0 : waitingRate)}
                                            onChange={(e) => {
                                        ...
                                        >
                                            {status === 'Normal' ? (
                                                <option value={1.0}>정상 청구 (100%)</option>
                                            ) : (
                                                <>
                                            ...
                                            <option value={-1}>직접 입력 (심야/인원초과/대기료 등)</option>
                                        </select>
                                    </div>
                                </div>

                                {/* Manual Input Fields (Hidden unless Direct Input selected) */}
                                {isManualWaiting && (
